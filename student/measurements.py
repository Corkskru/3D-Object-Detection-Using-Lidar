...
class Sensor:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if x <= 0:
                 print('Ignoring the negative x ')
            else :
                 #u = self.c_i - ((self.c_j * y)/x)
                 u = self.c_i - ((self.f_i * y)/x)
                 #v = self.c_j - ((self.c_j * z)/x)
            ...
            #insteead of returning hx, first reshape it 
            hx  = np.matrix(_hx.reshape(-1, 1))
            return hx
            ############
            # END student code
            ############ 
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.sensor = sensor
            cam_i = params.sigma_cam_i
            cam_j = params.sigma_cam_j
            self.R = np.matrix( [ [ cam_i**2 , 0], 
                                                        [0, cam_j**2]  ])
